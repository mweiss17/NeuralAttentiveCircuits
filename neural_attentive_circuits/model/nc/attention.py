...
@Tracer.register_tracer_probes("attention_weights", "attention_scores")
class ReadInAttention(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        head_dim: int = 64,
        include_residual: bool = True,
        layer_scale_initial_value: Optional[float] = 1.0,
        qkv_bias: bool = True,
        mod_fc_cls: str = "ModFC",
        mod_fc_kwargs: Optional[dict] = None,
        share_key_value_projectors: bool = False,
    ):
        ...
        self.include_residual = include_residual
        self.qkv_bias = qkv_bias
        self.share_key_value_projectors = share_key_value_projectors
        # Define the modules
        ...
        # The key and value compilers might need to be different, depending on whether we're
        # sharing key value projectors.
        if self.share_key_value_projectors:
            key_value_modfc_cls = utils.FC
            key_value_modfc_kwargs = {}
        else:
            key_value_modfc_cls = mod_fc_cls
            key_value_modfc_kwargs = mod_fc_kwargs
        self.key_compiler = key_value_modfc_cls(
            in_features=self.input_dim,
            out_features=self.inner_dim,
            context_features=self.code_dim,
            bias=self.qkv_bias,
            **(key_value_modfc_kwargs or {}),
        )
        self.value_compiler = key_value_modfc_cls(
            in_features=self.input_dim,
            out_features=self.inner_dim,
            context_features=self.code_dim,
            bias=self.qkv_bias,
    ...
    def forward(
        ...
        if not self.share_key_value_projectors:
        ...
        # keys.shape = values.shape = BVU(HC) or BV(HC)
        ...
        if self.share_key_value_projectors:
            # keys.shape = values.shape = BV(HC)
            head_unfolder_program = "b v (h c) -> b h v c"
        else:
            # keys.shape = values.shape = BVU(HC)
            head_unfolder_program = "b v u (h c) -> b h u v c"
        keys, values = map(
            lambda x: eo.rearrange(x, head_unfolder_program, h=self.num_heads),
            [keys, values],
        )
        ...
        # --------------------
        # Figure out which einsum program to use
        if self.share_key_value_projectors:
            attention_scorer_program = "bhuc,bhvc->bhuv"
        else:
            attention_scorer_program = "bhuc,bhuvc->bhuv"
        # Compute scaled dot product attention
        scale = math.sqrt(self.head_dim)
        attention_scores = torch.einsum(attention_scorer_program, queries, keys) / scale
        attention_scores = Tracer.track(self, "attention_scores", attention_scores)
        attention_weights = torch.softmax(attention_scores, dim=-1)
        # Track it
        attention_weights = Tracer.track(self, "attention_weights", attention_weights)
        # --------------------
        # Figure out which einsum program to use
        if self.share_key_value_projectors:
            weighter_program = "bhuv,bhvc->bhuc"
        else:
            weighter_program = "bhuv,bhuvc->bhuc"
        # Compute the aggregated messages from values
        aggregated_messages = torch.einsum(weighter_program, attention_weights, values)
        # Fold in the heads
        aggregated_messages = eo.rearrange(aggregated_messages, "b h u c -> b u (h c)")
        # --------------------
