...
class Kernel(nn.Module):
    ...
    def diagonal_elimination(self, kernel):
        if not self.eliminate_diagonal:
            return kernel
        # Make the mask
        # Can only eliminate diagonal in square kernels
        assert kernel.shape[-1] == kernel.shape[-2]
        extra_dims = kernel.dim() - 2
        diagonal_mask = torch.eye(
            kernel.shape[-1], dtype=torch.bool, device=kernel.device
        )[(None,) * extra_dims]
        # If we're returning log kernel, then kernel is in log space and we'll
        # need to fill the diagonals with -inf
        if self.returning_distance:
            # infinite distance between a vector and itself
            fill_value = float("inf")
        elif self.returning_log_kernel:
            # log(0) = -inf
            fill_value = -float("inf")
        else:
            # We're just returning the raw kernel in exp space
            fill_value = 0.0
        kernel = torch.where(
            diagonal_mask, torch.empty_like(kernel).fill_(fill_value), kernel
        )
        return kernel
...
class DotProductKernel(Kernel):
    ...
    def forward(
        ...
        # Eliminate diagonal if required
        kernel = self.diagonal_elimination(kernel)
        # Sample if required
